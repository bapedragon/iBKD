#!/usr/bin/env python3
"""Run the CUB ResNet50-224 scratch-teacher ablation and two baselines."""
...
RUN_NAMES = {
    "VanillaB128": "vanilla_cub200_224_lg_official_b128_300ep_seed1",
    "LG": "lg_cub200_resnet50_224_scratch_deit_ti_300ep_seed1",
    "ALG": "alg_cub200_resnet50_224_scratch_deit_ti_300ep_seed1",
    "VanillaB64": "vanilla_cub200_224_ours_current_b64_300ep_seed1",
    "Ours": "ours_cub200_resnet50_224_scratch_deit_ti_300ep_seed1",
}
METHOD_SCRIPTS = {
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path("/app/output/cub200_resnet50_224_scratch_all_seed1"),
    )
...
def format_final_top1_summary(best_top1: dict[str, float]) -> str:
    return (
        ...
        f"TeacherScratch224={best_top1['teacher']:.2f}% "
        f"VanillaB128={best_top1['VanillaB128']:.2f}% "
        f"LG={best_top1['LG']:.2f}% "
        f"ALG={best_top1['ALG']:.2f}% "
        f"VanillaB64={best_top1['VanillaB64']:.2f}% "
        f"Ours={best_top1['Ours']:.2f}%"
    )


def validate_scratch_teacher_summary(summary: dict[str, Any]) -> None:
    expected = {
        "pretrained": False,
...
def main() -> None:
    ...
    status: dict[str, Any] = {
        ...
        "protocols": {
            "teacher": (
                "random-init ResNet50, 224px, same recipe as transfer teacher"
            ),
            "VanillaB128": "official-LG-base DeiT-Ti, CE-only, batch 128",
            "LG": "official LG mechanics, scratch ResNet50-224 features",
            "ALG": "ALG paper controller, scratch ResNet50-224 features",
            "VanillaB64": "Ours-base DeiT-Ti, CE-only, batch 64",
            "Ours": "unchanged Ours, scratch ResNet50-224 features",
        },
    }
    atomic_json_save(status, status_path)
    ...
    log("=" * 96)
    log(
        "CUB-200 224 SCRATCH FAMILY: TEACHER -> VANILLA128 -> LG -> ALG "
        "-> VANILLA64 -> OURS"
    )
    log("=" * 96)
    log(f"[MODE] {mode}")
    ...
    log(
        "[CONTROLLED_ABLATION] relative_to_resnet50_224_transfer_family="
        "teacher_initialization_only"
    )
    log(
        "[VANILLA_BASELINES] LG/ALG_compare_to=VanillaB128 "
        "Ours_compare_to=VanillaB64 teacher_used=False"
    )
    log(
    ...
    validate_scratch_teacher_summary(summaries["teacher"])
    log(
        "[TEACHER_IDENTITY_CHECK] status=PASS pretrained=False "
        "model=resnet50_cub200_scratch_224 input=224"
    )

    task_specs = (
        ("VanillaB128", "lg_official_b128"),
        ("LG", None),
        ("ALG", None),
        ("VanillaB64", "ours_current_b64"),
        ("Ours", None),
    )
    for name, vanilla_profile in task_specs:
        run_name = RUN_NAMES[name] + suffix
        if vanilla_profile is not None:
            task_output = output_root / "Vanilla" / vanilla_profile
            command = [
                sys.executable,
            ...
            task_output = output_root / name / "cub200_resnet50_224_scratch"
            command = [
                sys.executable,
                METHOD_SCRIPTS[name],
                "--data-dir",
    ...
    atomic_json_save(status, status_path)
    log("=" * 96)
    log("[SEQUENCE_DONE_224_SCRATCH] completed_tasks=6/6")
    log(
        f"[POD_LIMIT_CHECK_224_SCRATCH] status={limit_status} "
    ...
    if timing and limit_status == "FAIL":
        log(
            "[NEXT_ACTION_224_SCRATCH] Split the six full tasks across Issues; "
            "reuse only the completed full scratch teacher, never the timing teacher."
        )
    log(f"[FINAL_RESULT_224_SCRATCH] sequence_status={status_path}")
...
if __name__ == "__main__":
    try:
        ...
        log(f"[FATAL_224_SCRATCH] {type(error).__name__}: {error}")
        raise
