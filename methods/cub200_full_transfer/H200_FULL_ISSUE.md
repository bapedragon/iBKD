...
이 본학습 Issue는 같은 commit의 timing run이 `completed_tasks=6/6` 및
`POD_LIMIT_CHECK_224_FULL_TRANSFER status=PASS`로 끝난 뒤 제출한다. 각
항목은 H200 Issue 입력란마다 각각 따로 복사한다.

## 제목

```text
[Request]: 박철현 CUB-200 pretrained Teacher 200ep + pretrained Vanilla-b128 + LG + ALG + Ours-b64/b128 300ep full training
```
