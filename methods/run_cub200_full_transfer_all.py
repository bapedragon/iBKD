#!/usr/bin/env python3
"""Run the fully pretrained CUB-200 teacher and five student experiments."""
...
METHOD_SCRIPTS = {
    "LG": (
        "methods/LG/cub200_resnet50_deit_ti_224_pretrained/train.py"
    ),
    "ALG": (
        "methods/ALG/cub200_resnet50_deit_ti_224_pretrained/train.py"
    ),
    "OursB64": (
        "methods/Ours/cub200_resnet50_deit_ti_224_pretrained/train.py"
    ),
    "OursB128": (
        "methods/Ours/cub200_resnet50_deit_ti_224_pretrained/train.py"
    ),
...
def format_final_top1_summary(best_top1: dict[str, float]) -> str:
    return (
        "[FINAL_TOP1_SUMMARY_224_FULL_TRANSFER] "
...
def validate_teacher_summary(summary: dict[str, Any]) -> None:
    ...
    if mismatches:
        raise RuntimeError(
            "Pretrained teacher identity validation failed: "
...
def validate_student_summary(
    ...
    if name == "VanillaB128":
        ...
        if teacher.get("pretrained") is not True:
    ...
    if mismatches:
        raise RuntimeError(
            f"{name} full-transfer identity validation failed: "
            f"{json.dumps(mismatches, sort_keys=True)}"
        )
...
def main() -> None:
    ...
    teacher_output = output_root / "teacher_resnet50_224_imagenet1k_v2"
    ...
    status: dict[str, Any] = {
        ...
        "protocols": {
            "teacher": (
                "ImageNet1K-V2 ResNet50, 224px, full CUB fine-tuning"
            ),
            "VanillaB128": (
                "ImageNet-1K DeiT-Ti, official-LG CE-only base, batch 128"
            ),
            "LG": (
                "ImageNet-1K DeiT-Ti, official LG mechanics, batch 128"
            ),
            "ALG": (
                "ImageNet-1K DeiT-Ti, ALG paper controller on official LG, "
                "batch 128"
            ),
            "OursB64": (
                "ImageNet-1K DeiT-Ti, unchanged Ours protocol, batch 64"
            ),
            "OursB128": (
                "ImageNet-1K DeiT-Ti, unchanged Ours except batch 128"
            ),
        },
    }
    atomic_json_save(status, status_path)
    ...
    log("=" * 104)
    log(
        "CUB-200 FULL TRANSFER: PRETRAINED TEACHER -> VANILLA128 -> LG -> "
        "ALG -> OURS64 -> OURS128"
    )
    log("=" * 104)
    log(f"[MODE] {mode}")
    log(
        ...
        "separate_from=teacher_only_transfer,resnet50_224_scratch,"
        "resnet56_32_scratch"
    )
    log(
    ...
    log(
        "[PRETRAINING_LOCK] teacher_pretrained=True "
        "all_students_pretrained=True "
        "teacher_source=torchvision.ResNet50_Weights.IMAGENET1K_V2 "
    ...
    log(
        "[OVERLAP_NOTICE] CUB-200 may overlap ImageNet; report this family "
        "only as a pretrained transfer experiment"
    )
    ...
    log(
        "[TEACHER_IDENTITY_CHECK] status=PASS pretrained=True "
        "source=torchvision.ResNet50_Weights.IMAGENET1K_V2 input=224"
    )
    ...
    for name in task_names:
        ...
        if name == "VanillaB128":
            task_output = output_root / "Vanilla" / "pretrained_b128"
            command = [
                sys.executable,
            ...
            task_output = output_root / name / "full_transfer"
            command = [
                sys.executable,
                METHOD_SCRIPTS[name],
                "--data-dir",
    ...
    atomic_json_save(status, status_path)
    log("=" * 104)
    log("[SEQUENCE_DONE_224_FULL_TRANSFER] completed_tasks=6/6")
    log(
        f"[POD_LIMIT_CHECK_224_FULL_TRANSFER] status={limit_status} "
    ...
    if timing and limit_status == "FAIL":
        log(
            "[NEXT_ACTION_224_FULL_TRANSFER] Split full tasks across Issues "
            "and reuse only the completed full teacher, never the timing teacher."
        )
    log(f"[FINAL_RESULT_224_FULL_TRANSFER] sequence_status={status_path}")
...
if __name__ == "__main__":
    try:
        ...
        log(
            f"[FATAL_224_FULL_TRANSFER] {type(error).__name__}: {error}"
        )
        raise
